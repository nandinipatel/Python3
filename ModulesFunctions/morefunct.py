#Author: Nandini Patel
#Date: May 08, 2020
#Description: Learning Functions using Parabola

import math

try:
    import tkinter
except ImportError: 
    import Tkinter as tkinter

def parabola(canvas, size):
    for x in range(-size, size):
        y =  x*x / 100 
        plot(canvas,x,y)

#when parameter is given a value, it means that default value will be used when nothing is passed for that parameter
def circle(canvas, radius, g, h, color="red"):
    canvas.create_oval(g + radius, h + radius, g - radius, h - radius, outline=color, width=2)
    # for x in range(g*100, (g+radius)*100):
    #     x /= 100
    #     y = h + (math.sqrt(radius ** 2 - ((x-g) ** 2)))
    #     plot(canvas, x, y)
    #     plot(canvas, 2 * g - x, y)
    #     plot(canvas, 2 * g - x, 2 * h - y)

def draw_axes(canvas):
    canvas.update() #to access width and height of the canvas
    x_origin = canvas.winfo_width() / 2
    y_origin = canvas.winfo_height() / 2
    canvas.configure(scrollregion=(-x_origin, -y_origin, x_origin, y_origin))
    canvas.create_line(-x_origin, 0, x_origin, 0, fill='black')
    canvas.create_line(0, y_origin, 0, -y_origin, fill="black")

def plot(canvas, x, y):
    canvas.create_line(x, -y, x+1, -y+1, fill="red")

#lets plot the values of the graph
mainWindow = tkinter.Tk()
mainWindow.title("Parabola")
mainWindow.geometry('640x480')
canvas = tkinter.Canvas(mainWindow, width = 640, height = 480)
canvas.grid(row=0, column=0)
draw_axes(canvas)

parabola(canvas, 100)
parabola(canvas, 50)
circle(canvas, 30, 0, 0, "green")
mainWindow.mainloop()